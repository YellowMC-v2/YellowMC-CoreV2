...
public class TriggerSyncEvent implements Listener {
    ...
    public void onPlayerQuit(PlayerQuitEvent event) {
        ...
        syncService.lockPlayerData(player.getUniqueId())
                .thenCompose(v -> syncService.savePlayerData(player.getUniqueId(), packet))
                .thenRun(() ->
                        YellowMCCoreV2.getInstance().getLogger().info(
                                "Saved sync data for player " + player.getName()
                        )
                )
                .exceptionally(ex -> {
                    YellowMCCoreV2.getInstance().getLogger().severe(
                            "Failed to save sync data for player " + player.getName()
                                    + ": " + ex.getMessage()
                    );
                    return null;
                });
